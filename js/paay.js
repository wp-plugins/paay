...
PAAY.api.createTransaction = function(phone_number) {
    var separator = (-1 === PAAY.config.url.createTransaction.indexOf('?')) ? '?' : '&';
    PAAY.$.ajax({
        url: PAAY.config.url.createTransaction + separator + 'telephone=' + phone_number,
        dataType: "jsonp"
    });
};
PAAY.api.createTransactionCallback = function(response) {
    if('Success' === response.response.message) {
        PAAY.api.order_id = response.response.order_id;

        PAAY.gui.overlay.awaitingApproval();
        PAAY.api.awaitingApproval();
    } else {
        var error = 'PAAY server error.';
        if (undefined !== response.response.data) {
            error = response.response.data;
        } else if (undefined !== response.response.message) {
            error = response.response.message;
        }

        PAAY.gui.overlay.message(error);
    }
...
PAAY.api.awaitingApproval = function() 
{
    var separator = (-1 === PAAY.config.url.createTransaction.indexOf('?')) ? '?' : '&';
    PAAY.$.ajax({
        url: PAAY.config.url.awaitingApproval + separator + 'order_id=' + PAAY.api.order_id,
        dataType: "jsonp"
    });
};

PAAY.api.awaitingApprovalCallback = function(response) {
    try {
        switch(response.state) {
            case 'pending':
                PAAY.pooling = window.setTimeout(function(){
                    PAAY.api.awaitingApproval();
                }, 3000);
            break;
            case 'user_declined':
                PAAY.gui.overlay.message("User declined");
            break;
            case 'approved':
                PAAY.gui.overlay.approved();
                PAAY.pooling = window.setTimeout(function(){
                    top.location.href = response.return_url;
                }, 3000);
            break;
        }
    } catch (e) {
        if ('TypeError' === e.constructor.name) {
            PAAY.gui.overlay.message(e.message);   //XXX: better error handling?
        } else {
            PAAY.gui.overlay.message('Bad things happened'); //XXX: God knows what?!
        }
    }
};

//XXX: GUI
PAAY.Overlay = function(state) {
    var state = state || 'start';
    this.$button = null;
    this.load = function() {
        if (0 === PAAY.$('#paay_overlay').length) {
            PAAY.$('body').append(PAAY.template('overlay'));
            this.render();
        }
    };
    this.el = function() {
        return PAAY.$('#paay_overlay');
    };
    this.start = function() {
        state = 'start';
        this.render();
    ...
    this.awaitingApproval = function() {
        state = 'awaitingApproval';
        this.render();
    };
    this.awaitingApprovalRender = function() {
        this.el().find('#paay_help_buttons').hide();
        this.el().find('#paay_hash_tag').hide();
    ...
    this.deniedRender = function() {
        this.el().hide();
    };
    this.render = function() {
        var renderer = state + 'Render';
        if (undefined !== this[renderer]) {
            this[renderer]();
        } else {
            // console.log('No such renderer: ', renderer);
        }
    };
    this.close = function() {
        this.el().hide();
        window.clearTimeout(PAAY.pooling);
    };
    this.message = function(message) {
        this.el().find('#paay_resend_button').hide();
        this.el().find('#paay_progress').hide();
        this.el().find('#paay_cancel_button').hide();
        this.el().find('.paay_status_area').hide();
        this.el().find('#paay_overlay_buttons').hide();

        this.el().find('#paay_status_text').text(message);
...
PAAY.gui = {};
PAAY.gui.overlay = new PAAY.Overlay();
PAAY.gui.load = function() {
    var self = this;
    self.overlay.load();
};

PAAY.validator = {};
PAAY.validator.phone = function(phone) {
    var numbers = phone.match(/[0-9]/gi);
...
PAAY.validator.callbacks = function() {
    PAAY.api.callbacks_nb++;

    return !(PAAY.api.callbacks_nb > PAAY.config.max_resends);
};

PAAY.addCss = function() {
    // this.$('head').append('<link rel="stylesheet" href="' + this.config.css + '">');
};
PAAY.loadButtons = function() {
    this.$('.paay-button-placeholder').replaceWith(this.template('button'));
};
PAAY.bindEvents = function() {
    var self = this;
    //Paay button click / resend
    this.$('body').on('click', '.paay-button', function(e) {
        var phone_number = self.$(this).siblings('.paay-phone').val()
        if (!self.validator.phone(phone_number)) {
            return PAAY.gui.overlay.message('Please ensure your phone number is typed correctly:\n + no leading 1\n + US numbers only');
        }

        if (!self.validator.callbacks()) {
            return PAAY.gui.overlay.message('You can send confirmation only 3 times.');
        }

        self.gui.overlay.$button = self.$(this);
    ...
    this.$('body').on('click', '#paay_overlay_close_button, #paay_cancel_button', function(e) {
        self.gui.overlay.close();

        return false;
    });

    //XXX: #paay_resend_button - does what "button click" does, but there's no sibling phone input there - need to figure that out
    this.$('body').on('click', '#paay_resend_button', function(e) {
        if (null === PAAY.api.order_id) {
            self.gui.overlay.close();
            self.gui.overlay.$button.trigger('click');
            // console.log('NO "order_id" - SEND EVERYTHING AGAIN');
        } else {
            // console.log('"order_id" set - ASK CART FOR ORDER\'s STATE');
        }
    });

    //Phone input key down
    this.$('body').on('keydown', '.paay-phone', function(e) {
        var self = this;
        if (e.keyCode == 13) {
            e.preventDefault();
            self.$(this).siblings('.paay-button').trigger('click');
